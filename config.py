...
После запуска программы в папке с исходными файлами для обработки создается новый файл Excel с двумя листами (_СВОД_анализ_счетов.xlsx): сводная таблица и таблица с отклонениями по оборотам до и после обработки. Второй лист служит для проверки правильности интеграции необходимых строк из анализов счетов в итоговый файл. При повторном запуске программы не забудьте убрать из папки файл _СВОД_анализ_счетов.xlsx
...
Например, для составления сводного отчета по инвестициям можно выгрузить анализ 08 счета по каждой компании с детализацией по строящимся объектам, статьям затрат и т.д. (до необходимого уровня детализации). Корреспондирующие счета могут быть расшифрованы по субсчетам, а счета 60 и 76 (или любые другие корр. счета) — по контрагентам или договорам, но не по контрагентам и договорам.
...
Также необходимо, чтобы анализ счета был расшифрован как минимум по субсчетам или субконто, иначе в 1С не будет сформирован столбец "Счет", без которого скрипт отработает неверно. Требование обязательности расшифровки корреспондирующих счетов нет, допускается по субсчетам и субконто первого уровня (не глубже). См. пример выше в «Тестирование на различных версиях 1С».
